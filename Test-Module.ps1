...
#!/usr/bin/env pwsh
# .SYNOPSIS
#   ilovepdfapi testScript v0.1.0
# .EXAMPLE
#   ./Test-Module.ps1 -version 0.1.0
#   Will test the module in ./BuildOutput/ilovepdfapi/0.1.0/
# .EXAMPLE
#   ./Test-Module.ps1
#   Will test the latest  module version in ./BuildOutput/ilovepdfapi/
param (
    ...
    [ValidateScript({
            ...
            $b_Path = [IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'ilovepdfapi')
...
begin {
    $TestResults = $null
    $BuildOutput = [IO.DirectoryInfo]::New([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'ilovepdfapi'))
    ...
    $BuildOutDir = Resolve-Path $([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'ilovepdfapi', $version)) -ErrorAction Ignore | Get-Item -ErrorAction Ignore
    if (!$BuildOutDir.Exists) { throw [System.IO.DirectoryNotFoundException]::new($BuildOutDir) }
    $manifestFile = [IO.FileInfo]::New([IO.Path]::Combine($BuildOutDir.FullName, "ilovepdfapi.psd1"))
    ...
    if (!(Test-Path -Path $([IO.Path]::Combine($PSScriptRoot, "ilovepdfapi.psd1")) -PathType Leaf -ErrorAction Ignore)) { throw [System.IO.FileNotFoundException]::New("Module manifest file Was not Found in '$($BuildOutDir.FullName)'.") }
    $script:fnNames = [System.Collections.Generic.List[string]]::New(); $testFiles = [System.Collections.Generic.List[IO.FileInfo]]::New()
    [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'ilovepdfapi.Integration.Tests.ps1')))
    [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'ilovepdfapi.Features.Tests.ps1')))
    [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'ilovepdfapi.Module.Tests.ps1')))
}
